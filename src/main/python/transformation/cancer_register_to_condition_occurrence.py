...
def cancer_register_to_condition_occurrence(wrapper: Wrapper) -> List[Wrapper.cdm.ConditionOccurrence]:
    ...
    df[topography10_columns] = df[topography10_columns].applymap(lambda x: x if x == 'NULL' else refactor_icdx_code(str(x)))
    df[topography9_columns] = df[topography9_columns].applymap(lambda x: x if x == 'NULL' else refactor_icdx_code(str(x)))
